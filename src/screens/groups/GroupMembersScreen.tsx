...
type GroupMembersRoute = RouteProp<RootStackParamList, 'GroupMembers'>;
type Navigation = NativeStackNavigationProp<RootStackParamList>;

export function GroupMembersScreen() {
  const { conversationId } = useRoute<GroupMembersRoute>().params;
  ...
  const isAdmin = entry.group?.myRole === 'admin';
  ...
  const renderItem = useCallback(
    ({ item }: { item: GroupMember }) => (
      <Pressable
        onLongPress={isAdmin && item.id !== currentUserId ? () => confirmRemove(item) : undefined}
        style={styles.row}
  ...
  return (
    <Screen>
      <FlatList
        ...
        ItemSeparatorComponent={() => <Separator inset={spacing.lg + 40 + spacing.md} />}
        ListHeaderComponent={
          isAdmin ? (
            <Pressable
