/**
 * Fil d'une conversation (ICH-019 à ICH-033).
 *
...
export function ChatScreen() {
  ...
  const conversationId = (conversation && conversation.id) || routeConversationId || null;
   ...
   * comme message. Le composant d'upload est celui partagé avec Adam, on ne lui
   * ajoute rien de spécifique à la messagerie.
   */
  const {
    uploading,
    progress,
    ...
    onUploaded: (attachment) => {
      ...
      void sendAttachment(conversationId, attachment, { replyToId: replyTo ? replyTo.id : null });
      setReplyTo(null);
    },
  });
  ...
  }, [conversation, conversationId, navigation, title]);
  ...
  const handleSend = useCallback(
    (text) => {
      ...
      void sendText(conversationId, text, replyTo ? replyTo.id : null);
      setReplyTo(null);
    },
  ...
  const handleLongPress = useCallback(
    (message) => {
      ...
      const actions = [{ text: 'Répondre', onPress: () => setReplyTo(message) }];
      ...
      Alert.alert('Message', undefined, actions, { cancelable: true });
    },
  ...
  const renderItem = useCallback(
    ({ item }) => {
      ...
      return (
        <MessageBubble
          ...
          showSender={item.showSender && !!conversation && conversation.type === 'group'}
    ...
    [conversation, currentUserId, handleLongPress, handleRetry, onPressFile, onPressImage]
  );
  ...
  if ((resolveError || status === 'error') && !items.length) {
    return (
      <Screen>
        <ErrorState
          message={resolveError || error || 'Conversation indisponible.'}
          onRetry={retry}
        />
      </Screen>
    );
