 ...
 * l'onglet, le badge de l'icône de l'app et les écrans d'Adam lisent tous ici.
 * On ne recalcule pas ce compteur ailleurs.
 */
import { create } from 'zustand';

import { conversationsApi, toUserMessage } from '@/services/api';
import type { Conversation, ConversationId, Message } from '@/types/models';
...
interface ConversationsState {
  ...
  /** Insère ou met à jour une conversation, puis retrie par activité. */
  upsert: (conversation: Conversation) => void;
  /** Répercute un message entrant ou sortant sur l'aperçu et les non-lus. */
  applyMessage: (message: Message, options?: { fromMe?: boolean; isActive?: boolean }) => void;
  setUnreadCount: (conversationId: ConversationId, unreadCount: number) => void;
...
export const useConversationsStore = create<ConversationsState>((set, get) => ({
  ...
  upsert(conversation) {
    ...
    const next = index >= 0 ? items.map((item, i) => (i === index ? { ...item, ...conversation } : item)) : [conversation, ...items];
  ...
  applyMessage(message, { fromMe = false, isActive = false } = {}) {
    ...
    const isNewer = !current.lastMessage || Date.parse(message.createdAt) >= Date.parse(current.lastMessage.createdAt);
...
export function selectTotalUnread(state: ConversationsState): number {
