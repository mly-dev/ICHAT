...
jest.mock('@/services/api', () => ({
  ...
  toUserMessage: (error: unknown) => (error instanceof Error ? error.message : 'erreur'),
}));

const list = messagesApi.list as jest.MockedFunction<typeof messagesApi.list>;
const remove = messagesApi.remove as jest.MockedFunction<typeof messagesApi.remove>;

function serverMessage(id: string, createdAt: string, senderId = 'u1'): Message {
  return {
    id,
    conversationId: 'c1',
    senderId,
...
describe('envoi optimiste', () => {
  it('affiche le message immédiatement en « en cours »', async () => {
    ...
    (messagesApi.send as jest.Mock).mockImplementation(() => new Promise(() => undefined));
  ...
  it('remplace le message optimiste par la version serveur, sans doublon', async () => {
    await useMessagesStore.getState().sendText('c1', 'bonjour');
    const clientId = selectThread(useMessagesStore.getState(), 'c1').items[0]?.clientId;
  ...
  it('repasse un message échoué en « en cours » au réessai', async () => {
    await useMessagesStore.getState().sendText('c1', 'bonjour');
    const clientId = selectThread(useMessagesStore.getState(), 'c1').items[0]?.clientId as string;
