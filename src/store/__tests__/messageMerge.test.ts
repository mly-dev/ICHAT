...
describe('ordonnancement des messages', () => {
  ...
  it('reste stable quand deux messages partagent le même horodatage', () => {
    ...
    const first = sortMessages([
      message({ id: 'z', createdAt: same }),
      message({ id: 'a', createdAt: same }),
    ]);
    const second = sortMessages([
      message({ id: 'a', createdAt: same }),
      message({ id: 'z', createdAt: same }),
    ]);
...
describe('déduplication', () => {
  it("n'ajoute pas deux fois le même id", () => {
    ...
    const merged = upsertMessage(existing, message({ id: 'a', createdAt: '2026-01-01T10:00:00.000Z', text: 'corrigé' }));
  ...
  it('supporte la réception du même message par HTTP puis par socket', () => {
    ...
    const merged = mergeMessages([fromHttp], [fromSocket, fromSocket]);
...
describe('statuts', () => {
  it('ne fait jamais redescendre un statut', () => {
    ...
    const merged = upsertMessage([read], lateDelivered);
