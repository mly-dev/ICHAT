...
import type { Conversation, Group, GroupMember } from '@/types/models';
...
describe('groupes', () => {
  it('ajoute le groupe créé à la liste des conversations', async () => {
    api.create.mockResolvedValueOnce(conversation);
  ...
  it('expose une erreur lisible si la création échoue', async () => {
    api.create.mockRejectedValueOnce(new Error('Le serveur est indisponible.'));
  ...
  it('charge le groupe et ses membres', async () => {
    api.getById.mockResolvedValueOnce(group);
    api.listMembers.mockResolvedValueOnce(members);
  ...
  it('répercute le nouveau nom sur la liste des conversations', async () => {
    ...
    api.update.mockResolvedValueOnce({ ...group, name: 'L3 Info 2026' });
  ...
  it('retire un membre tout de suite et revient en arrière si le serveur refuse', async () => {
    ...
    api.removeMember.mockRejectedValueOnce(new Error('Accès refusé.'));
  ...
  it('efface la conversation quand on quitte le groupe', async () => {
    useConversationsStore.getState().upsert(conversation);
    api.leave.mockResolvedValueOnce(undefined);
