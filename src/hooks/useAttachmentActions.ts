 ...
 */
export function useAttachmentActions(openImage: (attachment: MessageAttachment) => void) {
  const onPressImage = useCallback(
    (attachment: MessageAttachment) => openImage(attachment),
    [openImage]
  );

  const onPressFile = useCallback(async (attachment: MessageAttachment) => {
    try {
