...
import type { Conversation, ConversationId, UserId } from '@/types/models';

interface Params {
  conversationId?: ConversationId;
  /** Ouverture depuis un écran d'Adam : on n'a qu'un identifiant de membre. */
  peerId?: UserId;
}
 ...
 */
export function useResolvedConversation({ conversationId, peerId }: Params) {
  ...
  const [resolved, setResolved] = useState<Conversation | null>(known ?? null);
