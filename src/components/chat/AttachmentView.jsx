...
export const AttachmentView = memo(function AttachmentView({
  ...
  if (attachment.kind === 'image') {
    return (
      <Pressable
        onPress={() => onPressImage && onPressImage(attachment)}
  ...
  return (
    <Pressable
      onPress={() => onPressFile && onPressFile(attachment)}
...
const styles = StyleSheet.create({
  image: { width: 220, height: 220, borderRadius: radius.md, backgroundColor: colors.surfaceAlt },
  file: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.sm,
