...
describe('préférences de notification', () => {
  it('garde les valeurs par défaut quand le serveur est injoignable', async () => {
    notificationsApi.getPreferences.mockRejectedValueOnce(new Error('hors ligne'));
  ...
  it('applique la valeur du serveur', async () => {
    notificationsApi.getPreferences.mockResolvedValueOnce({
      enabled: false,
      directMessages: true,
      groupMessages: false,
    });
  ...
  it('revient en arrière si le serveur refuse la mise à jour', async () => {
    notificationsApi.updatePreferences.mockRejectedValueOnce(new Error('refus'));
  ...
  it('coupe toutes les alertes quand les notifications sont désactivées', async () => {
    notificationsApi.updatePreferences.mockResolvedValueOnce({
      enabled: false,
      directMessages: true,
      groupMessages: true,
    });
